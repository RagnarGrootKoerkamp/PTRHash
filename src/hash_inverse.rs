...
impl Inverter {
    ...
    #[cfg(test)]
    fn new_bruteforce(c: T, r: u32) -> Self {
    ...
    pub fn new(c: T) -> Self {
        ...
        for r in 1..T::BITS {
            diffs.push(Self::next_possible_diffs(c, r, diffs.last().unwrap()));
        }
        Self { diffs }
    }
    ...
    #[cfg(test)]
    fn xor_invert_bruteforce(x: T, r: u32) -> T {
        for k in 0.. {
    ...
    fn xor_invert(&self, x: T, r: u32) -> T {
        let mut k = 0;
        let mut rr = (C.wrapping_mul(k) ^ x).leading_zeros();
        'rr: while rr < r {
            for &d in &self.diffs[rr as usize] {
    ...
    /// Solve min_k { C*k = X ^ A : 0 <= A < 2^{64-r} } in $O(r)$.
    fn xor_invert_with_test(&self, x: T, r: u32, test: impl Fn(T) -> bool, mut k: u64) -> T {
        let mut rr = (C.wrapping_mul(k) ^ x).leading_zeros();
        'rr: while rr < r {
            for &d in &self.diffs[rr as usize] {
                let new_rr = (C.wrapping_mul(k + d) ^ x).leading_zeros();
        ...
        'l: loop {
            ...
            for &d in &self.diffs[r as usize] {
    ...
    pub fn invert_fr64(&self, x: Hash, n: usize, p: usize) -> u64 {
        ...
        if low == high {
            return self.xor_invert_with_test(
                low ^ x.get(),
                64, // r
                |k| {
        ...
        let k0 = self.xor_invert(low ^ x.get(), lcp);
        ...
        let low_k = self.xor_invert_with_test(low ^ x.get(), low_lcp, test, k0);
        let high_k = self.xor_invert_with_test(high_start ^ x.get(), high_lcp, test, k0);
...
mod test {
    ...
    fn test_find_diffs() {
        let r = 27;
        let diffs_1 = Inverter::new(C);
        let diffs_2 = Inverter::new_bruteforce(C, r);
        assert_eq!(&diffs_1.diffs[..=r as usize], &diffs_2.diffs[..]);
    }

    #[test]
    fn test_xor_inverse() {
        let inverter = Inverter::new(C);
        ...
        for i in 0..n {
            ...
            let k1 = inverter.xor_invert(x, r);
            let k2 = Inverter::xor_invert_bruteforce(x, r);
            assert_eq!(k1, k2);
        }
    ...
    fn test_invert_fr64() {
        ...
        let test = |its, maxn| {
            for i in 0..its {
                ...
                let k1 = inverter.invert_fr64(x, n, p);
                let k2 = Inverter::invert_fr64_bruteforce(x, n, p);
                assert_eq!(k1, k2);
            }
        };
